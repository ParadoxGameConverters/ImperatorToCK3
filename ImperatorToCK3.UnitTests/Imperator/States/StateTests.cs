...
public class StateTests {
	...
	private readonly ModFilesystem irModFS = new(ImperatorRoot, Array.Empty<Mod>());
	private readonly ProvinceCollection provinces = [];
	private readonly AreaCollection areas = [];
	private readonly CountryCollection countries = [new Country(69), new Country(70)];
	...
	public void StateCanBeInitialized() {
		...
		// Before we can check the capital province, we need to load the provinces.
		...
		provinces.LoadProvinces(provincesReader, stateCollection, countries, new MapData(irModFS));	
