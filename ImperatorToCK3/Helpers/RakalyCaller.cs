...
public static class RakalyCaller {
	public static void MeltSave(string savePath) {
		...
		if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX)) {
			executablePath = $"Resources/rakaly/rakaly-{rakalyVersion}-x86_64-apple-darwin/rakaly";
			Exec($"chmod +x {currentDir}/{executablePath}");
		} else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux)) {
			executablePath = $"Resources/rakaly/rakaly-{rakalyVersion}-x86_64-unknown-linux-musl/rakaly";
			Exec($"chmod +x {currentDir}/{executablePath}");
		}
