...
void Imperator::Mods::loadImperatorModDirectory(const Configuration& theConfiguration) {
	const auto& imperatorModsPath = theConfiguration.getImperatorDocsPath() + "/mod";
	if (!commonItems::DoesFolderExist(imperatorModsPath))
		throw std::invalid_argument("tImperator: Rome mods directory path is invalid! Is it at: " + theConfiguration.getImperatorDocsPath() + "/mod ?");
			...
			if (theMod.looksValid()) {
				if (!theMod.isCompressed()) {
					if (!commonItems::DoesFolderExist(theMod.getPath())) {
						...
						if (commonItems::DoesFolderExist(theConfiguration.getImperatorDocsPath() + "/" + theMod.getPath())) {
							// fix this.
							theMod.setPath(theConfiguration.getImperatorDocsPath() + "/" + theMod.getPath());
						} else {
...
std::optional<std::string> Imperator::Mods::getModPath(const std::string& modName) const {
...
bool Imperator::Mods::extractZip(const std::string& archive, const std::string& path) const {
